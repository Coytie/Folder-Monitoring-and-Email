...
$totalpath = $($abdpath, $abepath) + $obpath   # Total Folder Data Path
$totaldata = $nfp + "\SPTotalBackupData.csv"   # Total Combined Data of obdata and abdata
...
.DESCRIPTION
    The Get-ActiveBackupFolder function calculates the size of each subfolder within the specified path, excluding the specified folder.
...
.EXAMPLE
    Get-ActiveBackupFolder -Path "C:\YourFolderPath" -ExcludedPath "Old Backups" -OutputPath "C:\YourOutputPath"

    Description
    -----------
    This example calculates the size of each subfolder within "C:\YourFolderPath" excluding "Old Backups".
